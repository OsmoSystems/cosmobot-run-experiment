...
def mock_get_temperature_adc(mocker):
    ...
    mock_get_temperature_adc.return_value = Mock(
        value=sentinel.digital_count, voltage=sentinel.voltage
    )
...
class TestGetOrCreateTemperatureLog:
    def test_does_not_create_or_update_file_if_exists(self, tmp_path):
        ...
        mock_log_filepath = os.path.join(
            mock_experiment_directory, mock_temperature_log_filename
        )
...
class TestLogTemperature:
    def test_appends_temperature_reading_rows(self, tmp_path, mock_get_temperature_adc):
        ...
        module.log_temperature(
            mock_experiment_directory, capture_time_1, number_of_readings_to_average
        )
        module.log_temperature(
            mock_experiment_directory, capture_time_2, number_of_readings_to_average
        )

        expected_log_filepath = os.path.join(
            mock_experiment_directory, module.TEMPERATURE_LOG_FILENAME
        )
    ...
    def test_logs_averaged_reading_with_provided_capture_timestamp(
        self, tmp_path, mock_get_temperature_adc
    ):
        ...
        mock_get_temperature_adc.side_effect = [
            Mock(value=i, voltage=i) for i in range(number_of_readings_to_average)
        ]
    ...
    def test_logs_unaveraged_readings(self, tmp_path, mock_get_temperature_adc):
        ...
        expected_unaveraged_log_filepath = os.path.join(
            mock_experiment_directory, module.UNAVERAGED_TEMPERATURE_LOG_FILENAME
        )

        actual_readings = _get_log_readings(expected_unaveraged_log_filepath)

        unique_timestamps = {
            reading["capture_timestamp"] for reading in actual_readings
        }
