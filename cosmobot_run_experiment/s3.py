...
def sync_to_s3(local_sync_dir, additional_sync_params="", erase_synced_files=False):
    ...
    logging.info(
        "Performing sync of experiments directory: {local_sync_dir}".format(**locals())
    )
    ...
    s3_sync_dir = "s3://camera-sensor-experiments/{experiment_dir_name}".format(
        **locals()
    )

    command = (
        "/home/pi/.local/bin/aws s3 {s3_subcommand} {local_sync_dir} "
